...
function AdminUsers() {
  ...
  const refresh = async () => setUsers(await api.users.list());
  useEffect(() => {
    void refresh();
  }, []);
  ...
  const save = async (u: User) => {
    ...
    const currentUsers = await api.users.list();
    const exists = currentUsers.some((x) => x.id === u.id);
    if (
      !exists &&
      currentUsers.some((x) => x.email.toLowerCase() === u.email.toLowerCase())
    ) {
      alert(t("email_taken"));
      return;
    }
    if (exists) await api.users.update(u);
    else await api.users.create(u);
    setEditing(null);
    await refresh();
  };
    ...
    <div>
      ...
      <div className="border-t border-border">
        {users.map((u) => (
          <div
            key={u.id}
            className="grid grid-cols-[1fr_auto_auto] gap-6 items-center py-4 border-b border-border"
          >
            <div>
            ...
            <span
              className={`px-2 py-0.5 text-[9px] font-mono uppercase tracking-widest rounded-full ${u.role === "ADMIN" ? "bg-foreground text-background" : "border border-border text-muted-foreground"}`}
            >
              {u.role}
            </span>
...
function UserForm({
  ...
  return (
    ...
    >
      <div
        className="bg-background w-full max-w-md p-8 rounded-sm"
        onClick={(e) => e.stopPropagation()}
      >
        <h3 className="text-2xl font-display mb-6">
          {user.email ? t("edit") : t("new_user")}
        </h3>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          ...
          <F label={t("password")}>
            <input
              ...
              className="inp"
              placeholder={user.password ? "(unchanged)" : ""}
            />
          </F>
          <F label={t("role")}>
            <select
