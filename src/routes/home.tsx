...
function HomePage() {
  ...
  useEffect(() => {
    void Promise.all([api.posts.list(), api.collections.list()]).then(
      ([postItems, collectionItems]) => {
        setPosts(postItems.sort((a, b) => b.createdAt - a.createdAt));
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-accent/10 selection:text-accent">
      ...
      <main className="max-w-6xl mx-auto px-6">
        <section className="py-24 md:py-32 animate-reveal">
          <div className="max-w-2xl">
            <h1 className="text-5xl md:text-7xl font-display leading-[1.1] text-balance mb-8">
              {t("hero_title_pre")}{" "}
              <span className="italic">{t("hero_title_em")}</span>
              {t("hero_title_post")}
            </h1>
        ...
        <section
          className="pb-24 animate-reveal"
          style={{ animationDelay: "150ms" }}
        >
          <div className="flex items-end justify-between mb-12 border-b border-border pb-4">
            <h2 className="text-sm font-mono uppercase tracking-[0.2em] font-medium">
              {t("latest_posts")}
            </h2>
          </div>
          {posts.length === 0 ? (
  ...
  return (
    <footer className="border-t border-border py-12">
      <div className="max-w-6xl mx-auto px-6 flex justify-between items-center text-[10px] font-mono uppercase tracking-widest text-muted-foreground">
        <p className="italic font-display normal-case text-sm not-italic font-bold">
          EcchiAcademy
        </p>
